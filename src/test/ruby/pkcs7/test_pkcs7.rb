...
module PKCS7Test
  class TestPKCS7 < TestCase
    ...
    def create_signer_info_with_algo(algo)
      ...
      org.jruby.ext.openssl.impl.SignerInfoWithPkey.new(
        ...
        org.bouncycastle.asn1.pkcs.IssuerAndSerialNumber.new(org.bouncycastle.asn1.x500.X500Name.new('C=SE'),
                                                             BIG_ONE),
        algo,
    ...
    def assert_raise_pkcs7_exception
      yield
      raise 'expected PKCS7Exception to be raised but did not'
  ...
  class TestOpenSSL < TestCase
    ...
    def test_enveloped_add_recipient
      ...
      tmp.add_recipient(recipient_one)
      tmp.add_recipient(recipient_two)
      tmp.add_data(data)

      p7 = OpenSSL::PKCS7.new(tmp.to_der)
      assert_equal :enveloped, p7.type
      assert_equal [@ee1_cert.serial, @ee2_cert.serial].sort, p7.recipients.map(&:serial).sort
      assert_equal 2, p7.recipients.size
    ...
    def test_certificates_and_crls_setters
      p7 = OpenSSL::PKCS7.new
      p7.type = :signed

      crl = OpenSSL::X509::CRL.new(X509CRLString)

      p7.add_certificate(@ee1_cert)
      p7.add_crl(crl)
      assert_equal [@ee1_cert.serial], p7.certificates.map(&:serial)
      assert_equal 1, p7.crls.size

      p7.certificates = [@ca_cert]
      p7.crls = [crl]

      assert_equal [@ca_cert.serial], p7.certificates.map(&:serial)
      assert_equal 1, p7.crls.size
    end
