...
class TestKDF < TestCase
  ...
  def test_pbkdf2_hmac_sha256_c_20000_len_32
    omit_on_fips 'HMAC key must be >= 112 bits in approved mode'
    password = 'password'
  ...
  def test_pbkdf2_hmac_sha256_c_20000_len_32_fips
    omit_on_non_fips
    # FIPS requires HMAC key >= 112 bits (14 bytes)
    password = 'password-fips!!'
    salt = OpenSSL::Random.random_bytes(16)
    value1 = OpenSSL::KDF.pbkdf2_hmac(password, salt: salt, iterations: 20_000, length: 32, hash: 'sha256')
    value2 = OpenSSL::KDF.pbkdf2_hmac(password, salt: salt, iterations: 20_000, length: 32, hash: 'sha256')
    assert_equal value1, value2
  ...
  def test_pbkdf2_hmac_short_password_raises_in_fips
    omit_on_non_fips
    assert_raise(SecurityError) do
      OpenSSL::KDF.pbkdf2_hmac('password', salt: 'salt' * 4, iterations: 20_000, length: 32, hash: 'sha256')
    end
  end
  ...
  def test_hkdf_rfc5869_test_case_1
    omit_on_fips 'Key size for HMAC must be at least 112 bits in approved mode: SHA-256/HMAC'
